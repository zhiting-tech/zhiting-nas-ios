...
//        }
    ...
    func download(path: String) {
        ...
        goOperationQueue.async { [weak self] in
            ...
            self.downloadManager?.createFileDownloader(self.downloadUrlFromPath(path), headerStr: "{\"scope-token\":\"\(AreaManager.shared.currentArea.scope_token)\"}", pwd: pwd, userId: id, areaId: areaID ?? 0)
    ...
    func downloadDir(requestUrl: String, filePath: String) {
        ...
        goOperationQueue.async { [weak self] in
            ...
            self.downloadManager?.createDirDownloader(findApi, downloadApi: downloadApi, downloadPath: filePath, headerStr: "{\"scope-token\":\"\(AreaManager.shared.currentArea.scope_token)\"}", pwd: pwd, userId: id, areaId: areaID ?? 0)
        }
    ...
    func upload(urlPath: String, filename: String, tmpPath: String) {
        ...
        goOperationQueue.async { [weak self] in
            ...
            self.uploadManager?.createFileUploader("\(urlPath)\(filename)", filePath: filePath, fileName: filename, headerStr: "{\"scope-token\":\"\(AreaManager.shared.currentArea.scope_token)\"}", pwd: pwd, userId: id, areaId: areaID ?? 0)
        }
