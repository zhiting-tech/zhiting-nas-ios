...
//        reload()
    ...
    override func setupViews() {
        ...
        if isWriteRoot {
            headerView.setBtns(btns: [.upload,.newFolder])
        }else{
    ...
    func imagePickerController(_ picker: TZImagePickerController!, didFinishPickingPhotos photos: [UIImage]!, sourceAssets assets: [Any]!, isSelectOriginalPhoto: Bool) {
        ...
        if picker.allowPickingVideo {
            ...
            assets.forEach { asset in
                ...
                PHImageManager.default().requestPlayerItem(forVideo: phAsset, options: options) { [weak self] avPlayerItem, keys in
                    guard let self = self else { return }
                    if let item = avPlayerItem, let urlAsset = item.asset as? AVURLAsset {
                        
                        
                        GoFileNewManager.shared.upload(urlPath: "/plugin/wangpan/resources/\(self.currentPath)/", filename: "\(UUID().uuidString).\(urlAsset.url.pathExtension)", tmpPath: urlAsset.url.absoluteString)
